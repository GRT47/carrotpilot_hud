...
class ClusterThemeParamReader:
    def __init__(self) -> None:
        self._params = None
        try:
            from cluster_custom_params import CustomHudParams as Params
    ...
    try:
        ...
        while True:
            ...
            if theme_override is None and now >= next_theme_param_read:
                new_theme_mode = theme_param_reader.read() if theme_param_reader is not None else "auto"
                if not hasattr(renderer, '_last_param_theme_mode'):
                    renderer._last_param_theme_mode = new_theme_mode
                    if renderer.theme_mode != new_theme_mode:
                        renderer.set_theme_mode(new_theme_mode)
                elif new_theme_mode != renderer._last_param_theme_mode:
                    renderer.set_theme_mode(new_theme_mode)
                    renderer._last_param_theme_mode = new_theme_mode
