...
class ClusterSimulator:
    ...
    def update(self, command: SimulatorInput, dt: float) -> ClusterUiState:
        ...
        cruise_kph = min(int(speed_limit_kph + 12 + 5 * math.sin(self.elapsed)), int(MAX_SPEED_KPH))
        ...
        # Cycle gap from 1 to 4 every 2 seconds
        gap = int((self.elapsed / 2.0) % 4) + 1

        # Toggle cruise engaged / off every 5 seconds
        is_engaged = (self.elapsed % 10.0) < 5.0
        c_state = "engaged" if is_engaged else "off"
        ...
        return ClusterUiState(
            ...
            steering_output=self.steering_angle_deg,
            steering_output_normalized=self.steering,
            steering_output_kind="angle",
            speed_limit_kph=speed_limit_kph,
            cruise_override_kph=float(cruise_kph),
            cruise_override_color_mode=color_mode,
            cruise_override_label="MAX",
            speed_limit_source="sim",
            cruise_kph=float(cruise_kph),
            cruise_display_state=c_state,
            gear_text="D",
            cruise_gap=gap,
            lfa_active=True,
            custom_udp_data=self.udp_receiver.get_latest_data(),
