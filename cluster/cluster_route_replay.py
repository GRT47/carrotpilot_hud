...
class RouteLogParser:
    def __init__(
        ...
        self.ambient_light: float | None = None
    ...
    def _update_device_state(self, device_state: Any) -> None:
        ...
        ambient = safe_get(device_state, "lightSensor", None)
        if ambient is None:
            ambient = safe_get(device_state, "ambientLight", None)
        if ambient is not None:
            self.ambient_light = float(ambient)
...
def frame_to_state(frame: RouteReplayFrame) -> ClusterUiState:
    ...
    return ClusterUiState(
        ...
        ambient_light=frame.ambient_light,
    )
