...
LANE_TURN_SIGNAL_LEFT_CENTER_X = 960 - 260
LANE_TURN_SIGNAL_RIGHT_CENTER_X = 960 + 260
LANE_TURN_SIGNAL_CENTER_Y = 60
...
SIDE_GAUGE_TOP = 320
SIDE_GAUGE_BOTTOM = 418
SIDE_GAUGE_LOWER_TOP = 248  # Kept as is, though might be unused
SIDE_GAUGE_LOWER_BOTTOM = 346
SIDE_GAUGE_WIDTH = 62
SIDE_GAUGE_VALUE_Y = 296
SIDE_GAUGE_LABEL_OFFSET = 15
SIDE_GAUGE_LEFT_CENTER_X = 940
...
SYSTEM_PANEL_X = NAVI_LIVE_PANEL_X
SYSTEM_PANEL_Y = 0.0
SYSTEM_PANEL_W = NAVI_LIVE_PANEL_W
NAVI_LIVE_PANEL_Y = 1
...
class ClusterUiRenderer:
    def __init__(
        ...
        self._media_art_texture = None
        self._media_art_hash = ""
    ...
    def _current_theme(self) -> ClusterTheme:
        target_theme = current_cluster_theme(self.theme_mode, ambient_light=getattr(self, 'ambient_light', None))
        if self._theme.is_dark != target_theme.is_dark:
            self._prev_theme = getattr(self, '_current_interpolated_theme', self._theme)
            self._theme = target_theme
            self._theme_transition_start_time = time.monotonic()
            
        elapsed = time.monotonic() - self._theme_transition_start_time
        if elapsed < 1.0:
            self._current_interpolated_theme = interpolate_theme(self._prev_theme, self._theme, elapsed)
            return self._current_interpolated_theme
            
        self._current_interpolated_theme = self._theme
        return self._theme
    ...
    def render(self, state: ClusterUiState, signal_lights: tuple[bool, bool] | None = None) -> None:
        ...
        self.ambient_light = getattr(state, 'ambient_light', None)
        
        if state.custom_udp_data and "theme_mode" in state.custom_udp_data:
            self.set_theme_mode(state.custom_udp_data["theme_mode"])
    ...
    def _world_view_shift_x(self, state: ClusterUiState) -> float:
        ...
        right_panel_visible = bool(
            ...
            or self.screen_mode in (1, 2, 4, 5, 6)
        )
    ...
    def _draw_hud(self, state: ClusterUiState, signal_lights: tuple[bool, bool] | None = None) -> None:
        ...
        try:
            ...
            box_bounds = getattr(self, "_top_center_box", None)
            if box_bounds is not None:
                box_x, box_y, box_w, box_h = box_bounds
                signal_cy = box_y + box_h * 0.5
                # Reduce size by 20% (0.8 multiplier)
                signal_scale = (box_h / (TURN_SIGNAL_HEAD_HALF_HEIGHT * 2)) * 0.8
                
                # The turn signal is drawn relative to its center (cx). 
                # The tail extends backwards by 36 units (unscaled). 
                # We need the tail's edge to be clear of the box.
                padding = 24.0
                tail_offset = 36.0 * signal_scale
                
                left_cx = box_x - padding - tail_offset
                right_cx = box_x + box_w + padding + tail_offset
            ...
            # self._draw_custom_udp_info(state)
            self._draw_media_widget(state)
            self._draw_status_footer(state)
        finally:
            profile_stage = self._profile_start()
            rl.rl_pop_matrix()
    ...
    def _draw_status_footer(self, state: ClusterUiState) -> None:
        ...
        profile_stage = self._profile_start()
        self._draw_light_sensor_status(state)
        self._profile_add("hud.light_sensor", profile_stage)
    ...
    def _draw_git_status(self, state: ClusterUiState) -> None:
        status = state.git_status
        network_address = state.network_address
        actual_fps = state.actual_fps

        if status is None and network_address is None and actual_fps is None:
            return
        ...
        if status is not None:
            ...
            text_x = DESIGN_WIDTH - right_margin
            self._draw_text_with_stroke(text, text_x, center_y, text_size, WHITE, (5, 9, 12), 2, cache=True, anchor="right")
            
            text_w, _ = self._measure_text(text, text_size, spacing)
            dot_center_x = text_x - text_w - GIT_STATUS_DOT_TEXT_GAP - GIT_STATUS_DOT_RADIUS
            rl.draw_circle_v(rl.Vector2(dot_center_x, center_y), GIT_STATUS_DOT_RADIUS, rl_color(color))

        # 2. Network and FPS (Row above Git)
        network_y = center_y - row_h - 4.0
        current_x = DESIGN_WIDTH - right_margin
        
        # FPS
        if actual_fps is not None and math.isfinite(actual_fps):
            fps_text = f"FPS {actual_fps:.1f} Hz"
            fps_text = self._ellipsize_text(fps_text, text_size, FPS_STATUS_MAX_TEXT_W)
            self._draw_text_with_stroke(fps_text, current_x, network_y, text_size, WHITE, (5, 9, 12), 2, anchor="right")
            
            fps_w, _ = self._measure_text(fps_text, text_size, spacing)
            dot_center_x = current_x - fps_w - FPS_STATUS_DOT_TEXT_GAP - FPS_STATUS_DOT_RADIUS
            rl.draw_circle_v(rl.Vector2(dot_center_x, network_y), FPS_STATUS_DOT_RADIUS, rl_color(GREEN))
            current_x = dot_center_x - FPS_STATUS_DOT_RADIUS - 18.0
            
        # IP Address
        if network_address:
            network_text = self._ellipsize_text(network_address, text_size, GIT_STATUS_MAX_TEXT_W)
            self._draw_text_with_stroke(network_text, current_x, network_y, text_size, WHITE, (5, 9, 12), 2, cache=True, anchor="right")
            network_w, _ = self._measure_text(network_text, text_size, spacing)
            current_x -= (network_w + 12.0)
            
        # Wifi Icon
        if network_address or state.network_connected:
            ...
            self._draw_outlined_bottom_aligned_texture_icon(
                self._wifi_texture,
                current_x - icon_size * 0.5,
                network_y + icon_size * 0.5, # bottom aligned
                icon_size,
                icon_size,
                tint,
                alpha,
    ...
    def _draw_light_sensor_status(self, state: ClusterUiState) -> None:
        theme = self._current_theme()
        ...
        margin_right = 50.0
        margin_top = 50.0
        x = DESIGN_WIDTH - margin_right
        y = margin_top
        
        val_str = f"{state.light_sensor_percent}%" if state.light_sensor_percent is not None else "--%"
    ...
    def _draw_tesla_steering_wheel(self, center_x: float, center_y: float, radius: float, rotation_deg: float, color: tuple[int, int, int], alpha: int) -> None:
        rl.rl_push_matrix()
        rl.rl_translatef(center_x, center_y, 0.0)
        rl.rl_rotatef(rotation_deg, 0.0, 0.0, 1.0)
        
        rgba = rl_color((*color, alpha))
        
        # Outer ring
        thickness = radius * 0.16
        rl.draw_ring(rl.Vector2(0, 0), radius - thickness, radius, 0, 360, 36, rgba)
        
        # Center Hub
        hub_radius = radius * 0.22
        rl.draw_circle(0, 0, hub_radius, rgba)
        
        # Horizontal Spokes
        spoke_thickness = radius * 0.14
        rl.draw_rectangle(int(-radius + thickness), int(-spoke_thickness/2), int((radius - thickness)*2), int(spoke_thickness), rgba)
        
        # Vertical bottom Spoke
        rl.draw_rectangle(int(-spoke_thickness/2), 0, int(spoke_thickness), int(radius - thickness), rgba)
    ...
    def _draw_speed_block(self, state: ClusterUiState) -> None:
        ...
        # self._draw_speed_panel_bg() # Removed to keep only text
        
        # Vehicle speed is the only thing we draw in the bottom left now
        self._draw_text_with_stroke(
            speed_text,
        ...
        # Removed redundant cruise speed, driving mode, traffic, EV mode, and TPMS from bottom left 
        # as requested to keep the area clean and focused on speed only.
        
        # Draw Consolidated Top Center Info
        self._draw_top_center_info(state)
    ...
    def _draw_vector_car(self, x: float, y: float, w: float, h: float, color_tuple: tuple) -> None:
        c = rl_color(color_tuple)
        
        # 1. Main body
        body_rect = rl.Rectangle(x + w*0.05, y + h*0.4, w * 0.85, h*0.4)
        rl.draw_rectangle_rounded(body_rect, 0.4, 4, c)
        
        # 2. Front hood slope (right side)
        rl.draw_triangle(
            rl.Vector2(x + w * 0.9, y + h*0.4),
            rl.Vector2(x + w * 0.9, y + h*0.8),
            rl.Vector2(x + w, y + h*0.8),
            c
        )
        # Rear slope (left side)
        rl.draw_triangle(
            rl.Vector2(x + w * 0.05, y + h*0.4),
            rl.Vector2(x, y + h*0.8),
            rl.Vector2(x + w * 0.05, y + h*0.8),
            c
        )
        
        # 3. Cabin (Roof)
        roof_rect = rl.Rectangle(x + w*0.35, y, w*0.25, h*0.4)
        rl.draw_rectangle_rec(roof_rect, c)
        
        # Back windshield (left side of cabin)
        rl.draw_triangle(
            rl.Vector2(x + w*0.35, y),
            rl.Vector2(x + w*0.2, y + h*0.4),
            rl.Vector2(x + w*0.35, y + h*0.4),
            c
        )
        
        # Front windshield (right side of cabin)
        rl.draw_triangle(
            rl.Vector2(x + w*0.6, y),
            rl.Vector2(x + w*0.6, y + h*0.4),
            rl.Vector2(x + w*0.75, y + h*0.4),
            c
        )
        
        # 4. Wheels
        alpha = color_tuple[3] if len(color_tuple) == 4 else 255
        wheel_color = rl_color((20, 25, 30, alpha))
        wheel_radius = h * 0.22
        rl.draw_circle_v(rl.Vector2(x + w*0.25, y + h*0.8), wheel_radius, wheel_color)
        rl.draw_circle_v(rl.Vector2(x + w*0.75, y + h*0.8), wheel_radius, wheel_color)

    def _draw_top_center_info(self, state: ClusterUiState) -> None:
        center_x = self.width * 0.5 - self._world_view_shift_x(state)
        y = 70.0
        theme = self._current_theme()
        
        hda_green = (50, 255, 100)
        hda_pink = (255, 60, 180)

        is_engaged = state.cruise_display_state in ("enabled", "engaged")
        active_color = hda_green if is_engaged else theme.muted
        pink_color = hda_pink if is_engaged else theme.muted
        
        if self._cruise_set_visible(state) and state.cruise_override_kph is not None:
            max_text = str(int(round(state.cruise_override_kph)))
        else:
            max_text = "--"
            active_color = theme.muted
            pink_color = theme.muted

        # Estimate width of speed text
        char_w = 42.0
        speed_w = len(max_text) * char_w
        
        current_gear = (state.gear_text or "").strip().upper()[:1]
        if not current_gear:
            current_gear = "D"

        # Determine visibility
        show_limit = state.speed_limit_kph is not None or state.navi_debug is not None
        
        # Define widths and gaps
        limit_w = 64.0 if show_limit else 0.0
        limit_gap = 20.0 if show_limit else 0.0
        ...
        block_h = 18.0
        block_gap = 6.0
        gap_blocks_w = 4 * block_w + 3 * block_gap
        
        car_w = 42.0
        car_h = 22.0
        car_gap = 6.0
        car_group_w = gap_blocks_w + car_gap + car_w
        car_group_gap = 24.0
        ...
        total_w = limit_w + limit_gap + gear_w + gear_gap + car_group_w + car_group_gap + speed_w + speed_gap + auto_w
        
        # Start drawing from the left so the whole group is centered at center_x
        current_x = center_x - total_w * 0.5
        
        # Calculate bounding box
        box_padding_x = 24.0
        box_x = current_x - box_padding_x
        box_w = total_w + box_padding_x * 2
        box_y = y - 45.0
        box_h = 100.0
        
        # Draw background box (dark translucent with subtle border)
        self._rounded_rect(box_x, box_y, box_w, box_h, 16.0, (0, 0, 0, 150), (255, 255, 255, 40), 1.0)
        
        # 1. Speed Limit
        base_y = y + 5.0
        
        if show_limit:
            limit_center = current_x + limit_w * 0.5
            radius = limit_w * 0.5
            rl.draw_circle_v(rl.Vector2(limit_center, base_y), radius, rl_color(RED))
            rl.draw_circle_v(rl.Vector2(limit_center, base_y), radius - 4.0, rl_color(WHITE))
            limit_text = "--" if state.speed_limit_kph is None else str(state.speed_limit_kph)
            limit_font_size = 28 if len(limit_text) <= 2 else 22
            self._draw_text(limit_text, limit_center, base_y, limit_font_size, TEXT, anchor="center")
            current_x += limit_w + limit_gap
            
        # 2. Gear
        gear_center = current_x + gear_w * 0.5
        self._draw_text_with_stroke(current_gear, gear_center, base_y, 48.0, WHITE, (0, 0, 0), 2, anchor="center")
        current_x += gear_w + gear_gap
        
        # 3. Gap Blocks and Car
        gap_val = int(clamp(float(state.cruise_gap if state.cruise_gap is not None else 0), 0.0, 4.0))
        car_y = base_y - car_h * 0.5
        
        for i in reversed(range(4)):
            is_active = (i < gap_val)
            b_color = pink_color if is_active else (100, 100, 100, 150)
            bh = block_h - i * 2.0
            by = car_y + (car_h - bh)*0.5
            block_x = current_x + (3 - i) * (block_w + block_gap)
            self._rounded_rect(block_x, by, block_w, bh, 3.0, b_color, (0, 0, 0), 1.0)
            
        current_x += gap_blocks_w + car_gap
        
        car_color = (*WHITE, 255) if is_engaged else (*theme.muted[:3], 200)
        self._draw_vector_car(current_x, car_y, car_w, car_h, car_color)
        current_x += car_w + car_group_gap
        
        # 4. Target Speed
        speed_center = current_x + speed_w * 0.5
        speed_font_size = 76.0
        self._draw_text_with_stroke(max_text, speed_center, base_y, speed_font_size, active_color, (0, 0, 0), 2, anchor="center")
        current_x += speed_w + speed_gap
        
        # 5. AUTO and km/h
        auto_center = current_x + auto_w * 0.5
        is_auto_active = getattr(state, 'cruise_override_color_mode', 0) == 2
        auto_color = active_color if is_auto_active else theme.muted
        
        auto_y = base_y - 14.0
        self._rounded_rect(current_x, auto_y - 15.0, auto_w, auto_h, 6.0, (0, 0, 0, 0), auto_color, 2.0)
        self._draw_text_with_stroke("AUTO", auto_center, auto_y, 22.0, auto_color, (0, 0, 0), 1, anchor="center")
        self._draw_text_with_stroke("km/h", auto_center, base_y + 22.0, 26.0, active_color, (0, 0, 0), 1, anchor="center")
        
        self._top_center_box = (box_x, box_y, box_w, box_h)

    # Old _draw_cruise_gap_badge removed for Top Center layout
    ...
    def _draw_turn_signal(
        ...
        custom_cx: float | None = None,
        custom_cy: float | None = None,
        scale: float = 1.0,
    ) -> None:
        ...
        tail_back = -36 * scale
        tail_front = 12 * scale
        tail_half_height = 16 * scale
        head_tip_x = 60 * scale
        head_half_height = TURN_SIGNAL_HEAD_HALF_HEIGHT * scale
    ...
    def _draw_custom_udp_info(self, state: ClusterUiState) -> None:
        if not state.custom_udp_data:
            return
        
        # Display the custom UDP data in the top left corner (below the network status)
        y = 100.0
        x = 20.0
        
        if isinstance(state.custom_udp_data, dict):
            for key, value in state.custom_udp_data.items():
                if key in ("media_title", "media_artist", "media_art"):
                    continue
                text = f"{key}: {value}"
                self._draw_text_with_stroke(text, x, y, 20.0, WHITE, (0, 0, 0), 2, anchor="left")
                y += 25.0
        else:
            text = str(state.custom_udp_data)
            self._draw_text_with_stroke(text, x, y, 20.0, WHITE, (0, 0, 0), 2, anchor="left")

    def _draw_media_widget(self, state: ClusterUiState) -> None:
        if not state.custom_udp_data or not isinstance(state.custom_udp_data, dict):
            return
        title = state.custom_udp_data.get("media_title")
        artist = state.custom_udp_data.get("media_artist")
        art_base64 = state.custom_udp_data.get("media_art")
        
        if not title and not artist and not art_base64:
            return
        # Load and cache album art
        if art_base64 and art_base64 != self._media_art_hash:
            print(f"DEBUG: New album art received, length: {len(art_base64)}", flush=True)
            self._media_art_hash = art_base64
            if self._media_art_texture is not None:
                rl.unload_texture(self._media_art_texture)
                self._media_art_texture = None
            try:
                img_data = base64.b64decode(art_base64)
                print(f"DEBUG: Decoded img_data length: {len(img_data)}", flush=True)
                img = rl.load_image_from_memory(".jpg", img_data, len(img_data))
                print(f"DEBUG: Loaded image: {img.width}x{img.height}", flush=True)
                if img.width > 0:
                    self._media_art_texture = rl.load_texture_from_image(img)
                    print(f"DEBUG: Loaded texture ID: {self._media_art_texture.id} {self._media_art_texture.width}x{self._media_art_texture.height}", flush=True)
                rl.unload_image(img)
            except Exception as e:
                print(f"Error loading album art from memory: {e}", flush=True)
                print(f"Error loading album art: {e}", flush=True)
        elif not art_base64 and self._media_art_hash != "":
            print(f"DEBUG: art_base64 is empty!", flush=True)
        
        # Draw on the right side (where debug plot usually is)
        x_center = SYSTEM_PANEL_X + SYSTEM_PANEL_W / 2.0
        panel_h = DESIGN_HEIGHT - SYSTEM_PANEL_Y
        y_center = SYSTEM_PANEL_Y + panel_h / 2.0
        box_w = 720.0
        box_h = 280.0
        ...
        art_w = 0.0
        if self._media_art_texture is not None:
            # Draw album art on the left side of the box
            art_w = 220.0
            art_h = 220.0
            art_x = box_x + 30.0
            art_y = box_y + 30.0
            rl.draw_texture_pro(
                self._media_art_texture,
                rl.Rectangle(0, 0, self._media_art_texture.width, self._media_art_texture.height),
                rl.Rectangle(art_x, art_y, art_w, art_h),
                rl.Vector2(0, 0),
                0.0,
                rl_color(WHITE)
            )
        
        text_x = box_x + 60.0 + art_w
        max_text_w = box_w - (text_x - box_x) - 30.0

        sx = self.width / DESIGN_WIDTH
        sy = self.height / DESIGN_HEIGHT

        title_tw, _ = self._measure_text(title, 49.0, max(1.0, 49.0 * 0.02)) if title else (0.0, 0.0)
        artist_tw, _ = self._measure_text(artist, 32.0, max(1.0, 32.0 * 0.02)) if artist else (0.0, 0.0)
        
        max_tw = max(title_tw, artist_tw)
        
        import time
        now = time.perf_counter()
        pixels_per_sec = 40.0
        
        global_scroll_dist = max(0.0, max_tw - max_text_w + 40.0)
        global_scroll_duration = global_scroll_dist / pixels_per_sec if global_scroll_dist > 0 else 0.0
        pause_duration = 2.0
        cycle_duration = pause_duration * 2 + global_scroll_duration if global_scroll_dist > 0 else 1.0
        
        t = now % cycle_duration

        def _draw_scrolling_text(text_str: str, text_y: float, text_size: float, color, tw: float):
            if tw <= max_text_w:
                self._draw_text(text_str, text_x, text_y, text_size, color, anchor="left")
                return

            my_scroll_dist = tw - max_text_w + 40.0
            
            offset_x = 0.0
            if t > pause_duration:
                offset_x = -(t - pause_duration) * pixels_per_sec
                if offset_x < -my_scroll_dist:
                    offset_x = -my_scroll_dist

            rl.begin_scissor_mode(
                int(round(text_x * sx)),
                int(round(box_y * sy)),
                int(round(max_text_w * sx)),
                int(round(box_h * sy))
            )
            try:
                self._draw_text(
                    text_str, text_x + offset_x, text_y, text_size, color, anchor="left"
                )
            finally:
                rl.end_scissor_mode()
        ...
        # Title
        if title:
            _draw_scrolling_text(title, y_center - 60.0, 49.0, theme.text, title_tw)

        # Artist
        if artist:
            _draw_scrolling_text(artist, y_center + 10.0, 32.0, theme.muted, artist_tw)

        # Playback Progress
        position = state.custom_udp_data.get("media_position", 0)
        duration = state.custom_udp_data.get("media_duration", 0)
        ...
        if duration > 0:
            ...
            # Background bar
            rl.draw_rectangle(int(bar_x), int(bar_y), int(bar_w), int(bar_h), rl_color(theme.faint))
            
            # Progress bar
            progress = min(1.0, max(0.0, position / duration))
            rl.draw_rectangle(int(bar_x), int(bar_y), int(bar_w * progress), int(bar_h), rl_color(theme.text))
            ...
            time_str = f"{fmt_time(position)} / {fmt_time(duration)}"
            self._draw_text(time_str, bar_x + bar_w, bar_y - 14.0, 20.0, theme.text, anchor="right")
