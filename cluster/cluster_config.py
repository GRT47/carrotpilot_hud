...
def interpolate_color(c1: tuple, c2: tuple, progress: float) -> tuple:
    if progress <= 0.0: return c1
    if progress >= 1.0: return c2
    return tuple(int(a + (b - a) * progress) for a, b in zip(c1, c2))

def interpolate_theme(t1: ClusterTheme, t2: ClusterTheme, progress: float) -> ClusterTheme:
    if progress <= 0.0: return t1
    if progress >= 1.0: return t2
    
    kwargs = {}
    for field in t1.__dataclass_fields__:
        if field == 'name':
            kwargs[field] = t2.name if progress > 0.5 else t1.name
        elif field == 'is_dark':
            kwargs[field] = t2.is_dark if progress > 0.5 else t1.is_dark
        else:
            val1 = getattr(t1, field)
            val2 = getattr(t2, field)
            kwargs[field] = interpolate_color(val1, val2, progress)
            
    return ClusterTheme(**kwargs)
...
def current_cluster_theme(mode: object = "auto", now: float | None = None, ambient_light: float | None = None) -> ClusterTheme:
    ...
    # If ambient light (lightSensor) is available, use it instead of time.
    # Openpilot's light sensor returns values where a dark tunnel/night is usually < 50.
    if ambient_light is not None:
        if ambient_light < 50.0:
            return DARK_CLUSTER_THEME
        return LIGHT_CLUSTER_THEME

    # Fallback to local time if sensor is not available
